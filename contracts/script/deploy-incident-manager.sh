...
case "$NETWORK" in
    "hedera-testnet")
        ...
        CHAIN_ID="296"
        BLOCK_EXPLORER_URL="https://hashscan.io/testnet"
        ;;
    "hedera-mainnet")
...
if [ -n "$HEDERA_API_KEY" ]; then
    print_info "Verifying contract on HashScan..."
    ...
    if [ $? -ne 0 ]; then
        ...
        print_info "$BLOCK_EXPLORER_URL/contract/$CONTRACT_ADDRESS"
        print_info "Contract Address: $CONTRACT_ADDRESS"
    fi
else
    print_warning "HEDERA_API_KEY not provided, skipping verification"
fi
